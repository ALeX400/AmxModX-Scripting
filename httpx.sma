...
public test2(id)
{
	console_print(id, "your build is %d", VersionNum);
}
...
public plugin_precache() {
	...
	if ( read_flags(temp) & 1 )
		Download("github.com/ALeX400/AmxModX-Scripting/blob/master/httpx.sma", "", "HTTPX_VersionCheckComplete", "", 433, REQUEST_GET, "", "", 0, -1);
}
...
public DownloadThink(ent) {
	...
	if ( ! socket_is_readable(gInformation[gIndex][_hSocket], 1000) )
		return;

	if ( socket_is_readable(gInformation[gIndex][_Status] & STATUS_FIRSTRUN ) )
	{
